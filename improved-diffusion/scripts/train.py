...
def main_worker(rank,world_size):
    ...
    diffusion = SpacedDiffusion(
        ...
        pad_tok_id=args.pad_tok_id,
        loss_update_granu=args.loss_update_granu,
        schedule_update_stride=args.schedule_update_stride,
        save_dir=args.save_dir,
    )

    if getattr(args, 'adaptive_schedule_path', '') and os.path.exists(args.adaptive_schedule_path):
        if rank == 0:
            print(f"[AdaptiveNoise] Loading initial/resume schedule from: {args.adaptive_schedule_path}")
...
def create_argparser():
    ...
    text_defaults = dict(
        ...
        save_dir='../../checkpoints_new/adaptive_schedule_new',
