'''
Vendored from https://github.com/LuChengTHU/dpm-solver (the official DPM-Solver /
DPM-Solver++ reference implementation), the same way DiffuSeq-v2 vendors it as
`dpm_solver_pytorch.py`. `NoiseScheduleVP`, `model_wrapper`, `DPM_Solver`,
`interpolate_fn` and `expand_dims` below are unmodified upstream code (the
`sample()` method additionally carries the `x_start`/`input_ids_mask` partial-
conditioning hook that DiffuSeq added for their seq2seq infilling use case,
which TGM-DLM can reuse for its own masked-infill sampling later).

`TGMDLMModelWrapper` at the bottom of this file is new: a thin adapter that
speaks TGM-DLM's model calling convention (`model(x, t, desc_state, desc_mask,
**kwargs) -> x_start`) so `GaussianDiffusion.dpm_solver_sample_loop` (in
gaussian_diffusion.py) can drive this solver without touching the vendored
code above.
'''
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        ...
        with torch.no_grad():
            if method == 'adaptive':
                ...
                # NOTE: this branch fixes a bug introduced by DiffuSeq's
                # modification of the upstream LuChengTHU/dpm-solver code
                # (see https://github.com/Shark-NLP/DiffuSeq/tree/diffuseq-v2).
                # Their version did `x = self.model_fn(x, t)` where upstream
                # only populates `model_prev_list` — that reassignment
                # silently replaces the actual latent `x_t` with the model's
                # x0-prediction, which is then fed into
                # `multistep_dpm_solver_update` as if it were the latent.
                # That corrupts every step's ODE update. We restore the
                # correct upstream behavior (keep `x` as the true latent
                # throughout; only *read* `self.model_fn(x, t)` into
                # `model_prev_list`), while still supporting the optional
                # `x_start`/`input_ids_mask` partial-conditioning hook,
                # correctly gated so it's a no-op when `input_ids_mask is None`
                # (the default, used by full/unconditional-on-mask sampling).
                def _apply_mask(x):
                    if input_ids_mask is not None:
                        return torch.where(input_ids_mask == 0, x_start, x)
                    return x
...
# =============================================================================
#
# The vendored `model_wrapper` above (line ~181) is written for the generic
# `model(x, t_input, **model_kwargs)` / `model(x, t_input, cond, **model_kwargs)`
# calling conventions. TGM-DLM's `TransformerNetModel2` instead expects
# `model(x, t, desc_state, desc_mask, **model_kwargs)` (two separate
# conditioning tensors, not one `cond`), and it is always called *conditioned*
# (there is no classifier-free-guidance combination step, no unconditional
# forward pass). Rather than shoehorning that into the generic wrapper's
# `guidance_type` branches, this small adapter builds the noise-prediction
# `model_fn` that `DPM_Solver` expects directly.
#
# `GaussianDiffusion` already predicts x0 directly (`model_mean_type ==
# ModelMeanType.START_X`), so this wrapper's job is just:
#   1. convert the solver's continuous t in (0, 1] to the discrete/rescaled
#      timestep the model was trained on,
#   2. call the model with the desc conditioning,
#   3. apply the same `clip_denoised` / `denoised_fn` post-processing that
#      `GaussianDiffusion.p_mean_variance` applies, so behavior is consistent
#      with the existing `p_sample_loop` / `ddim_sample_loop` samplers,
#   4. convert the resulting x0 prediction back into a noise prediction
#      (the internal currency `DPM_Solver.model_fn` expects — see
#      `data_prediction_fn` above, which converts noise -> x0 for the
#      `dpmsolver++` branch. Doing the eps<->x0 conversion here rather than
#      relying on that internal conversion keeps this wrapper self-contained
#      and explicit about what the underlying model actually predicts).


def tgmdlm_model_wrapper(diffusion, model, noise_schedule, desc, clip_denoised=False,
                          denoised_fn=None, model_kwargs=None):
    """
    Build a noise-prediction `model_fn(x, t_continuous)` for `DPM_Solver`,
    wrapping a TGM-DLM `GaussianDiffusion` + model + text conditioning.

    Args:
        diffusion: a `GaussianDiffusion` (or `SpacedDiffusion`) instance. Only
            used for `model_mean_type`/`model_var_type`, `_predict_xstart_from_eps`,
            `_predict_eps_from_xstart`, and `_scale_timesteps` — the actual
            noise schedule driving the solver is `noise_schedule`, built
            separately from `diffusion.alphas_cumprod` (see
            `GaussianDiffusion.dpm_solver_sample_loop`).
        model: the denoising network, called as
            `model(x, t, desc_state, desc_mask, **model_kwargs) -> model_output`.
        noise_schedule: a `NoiseScheduleVP` built with
            `schedule='discrete', alphas_cumprod=diffusion.alphas_cumprod`.
        desc: a `(desc_state, desc_mask)` tuple of conditioning tensors.
        clip_denoised: same semantics as `GaussianDiffusion.p_mean_variance`.
        denoised_fn: same semantics as `GaussianDiffusion.p_mean_variance`.
        model_kwargs: extra kwargs forwarded to `model`.

    Returns:
        `model_fn(x, t_continuous) -> eps`, suitable for
    ...
    def get_model_input_time(t_continuous):
        # t_continuous in (0, 1] corresponds to discrete step index
        # i = round(t_continuous * total_N) - 1, since NoiseScheduleVP's
        # discrete grid is t_i = (i + 1) / N for i = 0 .. N-1 (see
        # NoiseScheduleVP.__init__ above). We recover that index, then apply
        # the same `_scale_timesteps` the rest of the codebase uses so the
        # model sees timesteps on the scale it was trained with.
        idx = torch.clamp(
            torch.round(t_continuous * total_N - 1), min=0, max=total_N - 1
        ).long()
        return idx
    ...
    def model_fn(x, t_continuous):
        ...
        model_output = model(x, t_input, desc_state.to(x.device), desc_mask.to(x.device),
                              **model_kwargs)

        # Mirror GaussianDiffusion.p_mean_variance's handling of a learned
        # variance head sharing the output tensor with the mean/x0 prediction.
        ...
        if diffusion.model_mean_type.name == "START_X":
            ...
            raise NotImplementedError(
                f"dpm_solver adapter does not support model_mean_type={diffusion.model_mean_type}"
            )

        if denoised_fn is not None:
            pred_xstart = denoised_fn(pred_xstart, t_idx)
        if clip_denoised:
            pred_xstart = pred_xstart.clamp(-1, 1)

        # DPM_Solver's internal currency is the noise prediction (see
        # `DPM_Solver.data_prediction_fn`, which converts noise -> x0 for
        # `algorithm_type="dpmsolver++"`). Convert our x0 prediction back to
        # an eps prediction using the *solver's* noise schedule (not the
        # discrete one on `diffusion`), since t_continuous may fall between
        # grid points during higher-order solver steps.
