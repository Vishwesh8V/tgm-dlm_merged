...
class TransformerNetModel2(nn.Module):
    def __init__(
        ...
        mask = False,
        learned_mean_embed=False,   # NEW — soft learned absorbing state
    ):
        super().__init__()
        ...
        # NEW — soft learned absorbing state (one vector shared across all
        # positions; only anchors the target sequence, never the frozen
        # SciBERT source encoding which is not diffused).
        if learned_mean_embed:
            self.mean_embed = nn.Parameter(th.randn(in_channels))
            nn.init.normal_(self.mean_embed, mean=0, std=in_channels ** -0.5)
        else:
